...
def login():
    # CSRF (Cross-site Reques Forgery): Solicictud de falsificacion entre sition
...
def registro():
    if request.method == "POST":
        ...
        tipousuario = request.form["tipousuario"]  # Este será siempre '2' (Cliente)

        # Verifica que las contraseñas coincidan
        if password != confirm_password:
            flash("Las contraseñas no coinciden. Por favor, intenta nuevamente.", "warning")
            return render_template("auth/register.html")

        # Lógica para crear el usuario en la base de datos
        nuevo_usuario = Usuario(None, usuario, password, tipousuario)
        try:
            ModeloUsuario.registrar(db, nuevo_usuario)  # Implementa esta función en tu modelo
    ...
    if current_user.is_authenticated:
        if current_user.tipousuario.id == 1:
            try:
                ...
                data = {"titulo": "Libros Vendidos", "libros_vendidos": libros_vendidos}
                return render_template("index.html", data=data)
            except Exception as ex:
                return render_template("error/error.html", mensaje=format(ex))
        else:
            try:
                compras = ModeloCompra.listar_compras_usuario(db, current_user)
                data = {"titulo": "Mis compras", "compras": compras}
                return render_template("index.html", data=data)
            except Exception as ex:
                return render_template("error/error.html", mensaje=format(ex))
    else:
...
def listar():
    try:
        ...
        data = {"titulo": "Listado de libros", "libros": libros}
